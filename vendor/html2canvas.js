...
_html2canvas.Parse = function (images, options) {
  ...
  function drawText(currentText, x, y, ctx, textShadow){
    if (currentText !== null && _html2canvas.Util.trimText(currentText).length > 0) {
      if (textShadow.indexOf('0px') > -1)
  ...
  function setTextVariables(ctx, el, text_decoration, color) {
    ...
    if (shadow && shadow != 'none') {
      var pixels = shadow.match(/(\d+)px/g)

      if (parseInt(pixels[2]) == 0) {
        var pixels = shadow.match(/(\d+)px/g)
        ctx.setVariable("strokeStyle", shadow.match(/rgba?\(([^\)]+)\)/)[0]);
        ctx.setVariable("lineWidth", parseInt(pixels[0]));
      } else {
        ctx.setVariable("shadowColor", shadow.match(/rgba?\(([^\)]+)\)/)[0]);
        ctx.setVariable("shadowOffsetX", parseInt(pixels[0]));
        ctx.setVariable("shadowOffsetY", parseInt(pixels[1]));
        ctx.setVariable("shadowBlur",    parseInt(pixels[2]));
      }
    }
  ...
  function renderText(el, textNode, stack) {
    ...
    textShadow = getCSS(el, "textShadow"),
    metrics,
    textList,
    state = {
      node: textNode,
    ...
    if (_html2canvas.Util.trimText(textNode.nodeValue).length > 0) {
      ...
      textList.forEach(function(text, index) {
        ...
        if (bounds) {
          drawText(text, bounds.left, bounds.bottom, ctx, textShadow);
  ...
  function supportRangeBounds() {
    ...
    if (doc.createRange) {
      ...
      if (r.getBoundingClientRect) {
        ...
        rangeHeight = Math.round(rangeBounds.height);
...
_html2canvas.Renderer.Canvas = function(options) {
  ...
  return function(zStack, options, doc, queue, _html2canvas) {
    ...
    ctx.globalAlpha = 1 / 25; // yf hack, fill canvas white transparent

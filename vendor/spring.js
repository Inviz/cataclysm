...
Spring.prototype.compute = function(now, from, REST_THRESHOLD, DISPLACEMENT_THRESHOLD) {
  ...
  this[2] = position;
  diff = position - old;
  if (Math.abs(diff + this[15]) > DISPLACEMENT_THRESHOLD) {
    this[7] = 1;
    this[15] = 0;
    return position;
  } else {
    this[15] += diff;
    if ((this[7] > 0 && Math.abs(this[6]) < REST_THRESHOLD) || 
       (Math.abs(position - goal) < DISPLACEMENT_THRESHOLD)) {
      this.clean();
...
Spring.prototype.complete = function(REST_THRESHOLD) {
  if (this[7] && Math.abs(this[6]) < REST_THRESHOLD) {
    this[7] = 0;
    return true;
  } else if (this[2] === this[3] && Math.abs(this[6]) < REST_THRESHOLD) {
    return true;
  }
};

Spring.prototype.STEP = 0.0005;

Spring.prototype.HALF = 0.00025;

Spring.prototype.MAX = 0.064;

Spring.prototype.DISPLACEMENT_THRESHOLD = 0.0001;

Spring.prototype.REST_THRESHOLD = 0.005;
...
Spring.Linear = function() {
  var range = [0, 1, null, 1, 0, 0, null, 0, 0, 0, 0, 0, 0, 0, 0, 0];
...
Spring.Linear.prototype = new Spring;
Spring.Linear.prototype.compute = function(animationTime, startTime, REST_THRESHOLD, DISPLACEMENT_THRESHOLD) {
  // register new goal
  if (this[6] == null || this[6] != this[3]) {
    this[0] = this[2]
    this[6] = this[3]
    this[7] = animationTime
  }
  this[5] = Math.min(1, (animationTime - this[7]) / 300)
  this[2] = this[0] + (this[3] - this[0]) * this[5]
  if (Math.abs(this[3] - this[2]) < DISPLACEMENT_THRESHOLD)
    this[2] = this[3]
  return this[2]
}
Spring.Linear.prototype.complete = function() {
  return this[5] == 1;
}
Spring.Linear.prototype.duration = 300;
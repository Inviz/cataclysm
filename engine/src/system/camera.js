...
P.Scene.onCameraMove = function(rotated) {
  if (rotated === true) {
    P.Person.instances.changes           |= P.UPDATE_ROTATION;
    P.Sprite.instances.changes           |= P.UPDATE_ROTATION;
    P.Label.instances.changes            |= P.UPDATE_ROTATION;
    P.Label.instances.front.changes      |= P.UPDATE_ROTATION;
    P.Panel.instances.changes            |= P.UPDATE_ROTATION;
    P.Icon.instances.changes             |= P.UPDATE_ROTATION;
    P.Icon.instances.front.changes       |= P.UPDATE_ROTATION;
    P.Background.instances.changes       |= P.UPDATE_ROTATION;
    P.Background.instances.front.changes |= P.UPDATE_ROTATION;
    P.Company.instances.changes          |= P.UPDATE_ROTATION;
    P.Pin.changes                        |= P.UPDATE_ROTATION;
  }
  P.Person.instances.changes           |= P.UPDATE_ALIGNMENT;
  P.Sprite.instances.changes           |= P.UPDATE_ALIGNMENT;
  P.Label.instances.changes            |= P.UPDATE_ALIGNMENT;
  P.Label.instances.front.changes      |= P.UPDATE_ALIGNMENT;
  P.Panel.instances.changes            |= P.UPDATE_ALIGNMENT;
  P.Icon.instances.changes             |= P.UPDATE_ALIGNMENT;
  P.Icon.instances.front.changes       |= P.UPDATE_ALIGNMENT;
  P.Background.instances.changes       |= P.UPDATE_ALIGNMENT;
  P.Background.instances.front.changes |= P.UPDATE_ALIGNMENT;
  P.Company.instances.changes          |= P.UPDATE_ALIGNMENT;
  P.Pin.changes                        |= P.UPDATE_ALIGNMENT;
  
}
P.Scene.needsUpdate = function(hard) {
  P.Sprite.instances.changes      |= P.UPDATE_RESET;

  P.Overlay.instances.changes     |= P.UPDATE_CULLING | P.UPDATE_COLOR;
  P.Underlay.instances.changes    |= P.UPDATE_CULLING | P.UPDATE_COLOR;
  P.Floor.instances.changes       |= P.UPDATE_CULLING | P.UPDATE_COLOR;

  // allow objects to appear if necessary
  P.Background.instances.changes  |= P.UPDATE_CULLING;
  P.Wall.instances.changes        |= P.UPDATE_CULLING;
  P.Overlay.instances.changes     |= P.UPDATE_CULLING;
  P.Underlay.instances.changes    |= P.UPDATE_CULLING;
  P.Person.instances.changes      |= P.UPDATE_CULLING;
  P.Panel.instances.changes       |= P.UPDATE_CULLING;
  P.Label.instances.changes       |= P.UPDATE_CULLING;
  P.Label.instances.front.changes |= P.UPDATE_CULLING;
  P.Icon.instances.changes        |= P.UPDATE_CULLING;
  P.Icon.instances.front.changes  |= P.UPDATE_CULLING;
  P.Furniture.instances.changes   |= P.UPDATE_CULLING;
  P.Line.instances.changes        |= P.UPDATE_CULLING;
  P.Company.instances.changes     |= P.UPDATE_CULLING;
  P.Pin.changes                   |= P.UPDATE_CULLING;
}
...
P.Scene.onPerspectiveChange = function(type) {
  for (var name in P.Wall.types) {
    var material = P.Wall.types[name];
    if (material.isGlass) {
      material.opacity = type == 'flat' ? 0.8 : 0.5;
      material.color = type == 'flat' ? P.styles.glassColor2 : P.styles.glassColor;
    }
  }
  if (P.Scene.state !== 'editor') {
    if (type === 'flat') {
      P.Floor.instances.material.color = new THREE.Color(244 / 255,244 / 255,244 / 255)
    } else {
      P.Floor.instances.material.color = new THREE.Color(239 / 255,239 / 255,239 / 255)
    }
  }
  P.Wall.instances.changes |= P.Wall.instances.UPDATE_OPACITY
  P.Wall.instances.changes |= P.Wall.instances.UPDATE_COLOR
  P.Furniture.instances.changes |= P.Wall.instances.UPDATE_OPACITY
  P.Furniture.instances.changes |= P.Wall.instances.UPDATE_COLOR
  P.Floor.instances.changes |= P.Wall.instances.UPDATE_COLOR
}

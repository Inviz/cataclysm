...
function onWindowResize(force) {
  clearTimeout(P.Scene.resizingWindow1);
  if (force !== false) {
    var i = 0;
  ...
  if (force || wWidth !== window.innerWidth || wHeight !== window.innerHeight) {
    ...
    if (P.Scene.current && P.Scene.current.makeScreenshot)
      P.Scene.current.makeScreenshot()
    ...
    P.animate.unlock()
    if (P.Panel.expanded && !P.Panel.expanded.loading)
...
function render(force) {
  ...
  if (P.Scene.current)
    renderer.setClearColor(
      P.styles.currentBackgroundColor.copy(camera));
  renderer.clear()
